...
"""
Tests for combining ``nargs`` with ``parser='csv'``/``'yaml'``.

The rule (uniform, no special cases): ``nargs`` applies the field's parser to
EACH token and **collects** the results into a list. We just use the real
parser per token; argparse collects into a list. No concat, no flatten, no
parser-kind distinction, no annotation depth-matching.
...
(spaces), not both. The scriptconfig dual-form (``--k a,b,c`` == ``--k a b c``)
is intentionally NOT supported -- it required a concat special-case and was
ambiguous for structured tokens (``--k [1,2,3] [4,5,6]`` -> flatten or group?).
