...
def _warn_user(message: str, category=UserWarning) -> None:
    ...
    Hot/cold and Python/Rust backend paths intentionally traverse different
    internal modules.  A fixed ``stacklevel`` therefore leaks backend-specific
    filenames into otherwise identical warnings.  Resolve the first external
    frame dynamically so diagnostics stay user-facing and backend-invariant.
...
def _has_cli_lifecycle_override(cls: type) -> bool:
    """Return True when a subclass expects the generic load/argv hooks."""
    config_type = globals().get('Config')
    for base in cls.__mro__:
        if base is config_type:
            break
        namespace = base.__dict__
        if any(name in namespace for name in ('_load', '_read_argv', 'argparse', '_argparse')):
            return True
    return False


def _try_direct_rust_cli(
    self: 'Config',
    argv: Any,
    *,
    strict: bool,
    autocomplete: bool | str,
    special_options: bool | None,
    validate: bool | str | None,
) -> bool:
    """Handle the ordinary flat CLI lifecycle without loading cold helpers.

    This path is deliberately narrower than :meth:`Config.load`. A miss is
    side-effect free and immediately delegates to the canonical generic path.
    """
    if _has_cli_lifecycle_override(type(self)):
        return False
    if special_options is None:
        special_options = getattr(self, '__special_options__', False)
    active_completion = bool(autocomplete) and '_ARGCOMPLETE' in os.environ
    if autocomplete is True or active_completion:
        if active_completion:
            from kwconf import _completion

            if _completion.try_config_argcomplete(
                self,
                autocomplete=autocomplete,
                special_options=bool(special_options),
            ):
                return False  # pragma: no cover - completion exits
        # autocomplete=True promises the canonical argcomplete integration even
        # outside an active shell request. Complex active requests also land
        # here after the static Rust completer conservatively declines.
        return False
    if special_options:
        # Special-option execution (config-file loading / dumps) remains owned
        # by the canonical Python lifecycle, but their static names can still
        # be completed above without constructing argparse.
        return False

    backend_mode = os.environ.get(
        'KWCONF_CLI_BACKEND', getattr(self, '__cli_backend__', 'auto')
    ).lower()
    ...
    # Match the canonical ``load(_reset=True)`` lifecycle only after Rust has
    # proven that it can own this parse.  The dry run above is important: a
    # fallback must remain side-effect free for default factories and custom
    ...
    if getattr(self, '_has_subconfigs', False):
        ...
        _subcfg_mod.distribute_explicit_argv_keys(self, explicit_keys)
        _subcfg_mod.distribute_provided_keys(self, explicit_keys)
...
class Config(NiceRepr, _ABCMapping, metaclass=MetaConfig):
    ...
    # Prefer the optional accelerator when installed; pure-Python wheels and
    # unsupported schemas transparently retain the canonical argparse path.
    __cli_backend__: str = 'auto'
    ...
    def cli(
        ...
        used_direct_rust = False
        if data is None and not default and argv is not False:
            used_direct_rust = _try_direct_rust_cli(
                self,
                argv,
                strict=strict,
                autocomplete=autocomplete,
