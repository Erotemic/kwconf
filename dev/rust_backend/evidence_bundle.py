...
def _make_cli() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--completion-trials', type=int, default=50)
    parser.add_argument('--help-trials', type=int, default=30)
    parser.add_argument(
        '--kwconf-rs',
        default='auto',
        help=(
...
def main() -> None:
    ...
    run_id = _now_id()
    if args.quick:
        # Keep this mode genuinely diagnostic/interactive even on slow or
        # instrumented Python builds. The full default campaign is the release
        # evidence run; --quick only proves that every layer is wired up and
        # that a bundle is produced.
        # cli_startup requires at least two observations; two completion
        # observations also catch unstable wire output without turning the
        # quick bundle into a performance campaign.
        args.startup_trials = 2
        args.completion_trials = 2
        args.help_trials = 1
