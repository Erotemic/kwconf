...
from kwconf.config import Config, MetaConfig, _materialize_default_items
from kwconf.value import Value
import warnings
import ubelt as ub
from kwconf import diagnostics
from kwconf.subconfig import SubConfig, wrap_subconfig_defaults
...
class MetaDataConfig(MetaConfig):
    ...
    def __new__(mcls: type,
        ...
        if namespace.get('__module__', None) != 'kwconf.dataconfig' or name != 'DataConfig':
...
class DataConfig(Config, metaclass=MetaDataConfig):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if getattr(self, '__default__', None):
            self._default.update(_materialize_default_items(self.__default__))
